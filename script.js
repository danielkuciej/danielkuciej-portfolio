...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Funkcja otwierania modala
  function openModal(fullImagePath) {
    // Pokaż modal NATYCHMIAST z rozmytym placeholderem
    modal.classList.add("is-visible");
    document.body.classList.add("body-no-scroll");

    // Dodaj klasę rozmycia (na wypadek, gdyby została usunięta wcześniej)
    modalImage.classList.add("lazy-img");

    // Rozpocznij pobieranie ciężkiego obrazka
    modalImage.src = fullImagePath;
  }
  ...
  // Nasłuchiwanie na kliknięcie w portfolio
  portfolioTriggers.forEach((trigger) => {
    trigger.addEventListener("click", function () {
      const fullImage = this.dataset.fullImage;
      openModal(fullImage);
